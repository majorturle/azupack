...
fn test_login_error() {
    ...
    let is_err = provider.get_token("https://dev.azure.com/nonexistant")
        .is_err();
...
fn test_login_single() {
    ...
    let result = provider.get_token("https://dev.azure.com/LlamaCo").unwrap();
    ...
    let result = provider.get_token("https://dev.azure.com/TurtleCo").unwrap();
    assert_eq!(result.len(), 1);
